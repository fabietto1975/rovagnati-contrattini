...
class SecurityAspect implements Aspect {
    ...
    public function checkAuthorization(MethodInvocation $invocation) {
        include \Go\Instrument\Transformer\FilterInjectorTransformer::rewrite((APP_ROOT . '/config/config-local.php'), 'D:\xampp-5.6.3\htdocs\rilevazioni\backend\app\aop\aspects');
        if ($token['check'] == 'true') {
